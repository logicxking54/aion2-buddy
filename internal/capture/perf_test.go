...
func BenchmarkEditCompressedCasts(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		copy(raw, payload)
		e.editCompressedCasts(raw, raw, 0, scanIDs, scanFB, lookup, idToName, 0, false, false, 0, 0)
	}
}
...
func BenchmarkInboundSession(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for _, p := range payloads {
			buf := raw[:len(p)]
			copy(buf, p)
			if handled, _ := e.editCompressedCasts(buf, buf, 0, scanIDs, scanFB, lookup, idToName, 0, false, false, 0, 0); !handled {
